...
def run_evaluation(project_dir, use_large=False):
    ...
    # 12. Comparative Analysis: PINN vs Traditional Methods in Out-of-Bounds (Unsampled) Regimes
    print("Generating comparative analysis plots for PINN vs Traditional Methods...")
    ...
    # Compute Classical Potential Flow Theory
    cl_pot = 2.0 * np.pi * alphas_ext
    cd_pot = np.zeros_like(alphas_ext)
    
    # Extract CFD Reference Points for Re = 1e6
    cfd_alpha = []
    cfd_cd = []
    cfd_cl = []
    if '1000000' in raw_data:
        cfd_alpha = np.array(raw_data['1000000']['alpha'])
        cfd_cd = np.array(raw_data['1000000']['Cd'])
        cfd_cl = np.array(raw_data['1000000']['Cl'])
    
    fig_comp, axes_comp = plt.subplots(1, 2, figsize=(14, 6))
    
    # Left Subplot: Drag Coefficient Cd comparison
    axes_comp[0].plot(alphas_ext, cd_pinn_ext, color='#185FA5', linewidth=2.5, label='PINN (Proposed)')
    axes_comp[0].plot(alphas_ext, cd_pot, color='#E24B4A', linestyle='--', linewidth=2.0, label="Potential Flow ($Cd=0$)")
    if len(cfd_alpha) > 0:
        axes_comp[0].scatter(cfd_alpha, cfd_cd, color='black', marker='o', s=40, zorder=5, label='Traditional CFD (Karabelas 2012)')
    axes_comp[0].axhline(y=0.0, color='black', linestyle='-', linewidth=1.0)
    ...
    # Right Subplot: Lift Coefficient Cl comparison
    axes_comp[1].plot(alphas_ext, cl_pinn_ext, color='#1D9E75', linewidth=2.5, label='PINN (Proposed)')
    axes_comp[1].plot(alphas_ext, cl_pot, color='#E24B4A', linestyle='--', linewidth=2.0, label='Potential Flow ($Cl=2\\pi\\alpha$)')
    if len(cfd_alpha) > 0:
        axes_comp[1].scatter(cfd_alpha, cfd_cl, color='black', marker='o', s=40, zorder=5, label='Traditional CFD (Karabelas 2012)')
    ...
    axes_comp[1].set_ylim([-15.0, 15.0])
    ...
    print("\n" + "="*80)
    print("SUMMARY: PINN VALIDATION METRICS VS TRADITIONAL CFD")
    print("="*80)
